...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'VetPage' });
  const title = t('metaTitle');
  const description = t('metaDesc');
  const keywords = locale === 'es'
    ? ['web para veterinarias', 'diseño web veterinario', 'citas mascotas', 'marketing veterinario']
    : ['vet clinic websites', 'veterinary web design', 'pet appointment booking', 'vet marketing'];
  return {
    title,
    description,
    keywords,
    alternates: {
      canonical: `${siteUrl}/${locale}/industries/vets`,
      languages: { en: `${siteUrl}/en/industries/vets`, es: `${siteUrl}/es/industries/vets`, "x-default": `${siteUrl}/en/industries/vets` },
    },
    ...ogMeta({ locale, title, description, path: '/industries/vets' }),
  };
}

export default async function VetIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'VetPage' });
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href={`${siteConfig.whatsapp.link}?text=${encodeURIComponent('Hola, me interesa una página web para mi veterinaria')}`}
            target="_blank"
      ...
      <RelatedIndustries locale={locale} current="vets" />
      <InternalNav locale={locale} exclude={['/industries']} />
      <Footer />
      <FloatingSocialButtons />
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/vets' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbVet'), url: `${siteUrl}/${locale}/industries/vets` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
