...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const keywords = locale === 'es'
    ? ['web para abogados', 'diseño web despacho legal', 'marketing legal', 'agenda de consultas']
    : ['law firm websites', 'attorney web design', 'legal marketing', 'consultation booking'];
  return {
    title,
    description,
    keywords,
    ...
    ...ogMeta({ locale, title, description, path: '/industries/legal' }),
  };
}
...
export default async function LegalIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="relative min-h-screen bg-background">
      ...
      <RelatedIndustries locale={locale} current="legal" />
