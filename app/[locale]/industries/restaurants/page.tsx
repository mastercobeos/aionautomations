...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const keywords = locale === 'es'
    ? ['web para restaurantes', 'menú digital', 'reservaciones en línea', 'marketing restaurantes']
    : ['restaurant websites', 'digital menu', 'online reservations', 'restaurant marketing'];
  return {
    title,
    description,
    keywords,
    alternates: {
      canonical: `${siteUrl}/${locale}/industries/restaurants`,
      languages: { en: `${siteUrl}/en/industries/restaurants`, es: `${siteUrl}/es/industries/restaurants`, "x-default": `${siteUrl}/en/industries/restaurants` },
    },
    ...ogMeta({ locale, title, description, path: '/industries/restaurants' }),
  };
}
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href={`${siteConfig.whatsapp.link}?text=${encodeURIComponent('Hola, me interesa una página web para mi restaurante')}`}
            target="_blank"
      ...
      <RelatedIndustries locale={locale} current="restaurants" />
      <InternalNav locale={locale} exclude={['/industries']} />
      <Footer />
      <FloatingSocialButtons />
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/restaurants' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbRestaurants'), url: `${siteUrl}/${locale}/industries/restaurants` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
