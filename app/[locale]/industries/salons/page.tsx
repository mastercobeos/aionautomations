...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const keywords = locale === 'es'
    ? ['web para salones', 'diseño web salón de belleza', 'reserva de citas', 'marketing salones']
    : ['salon websites', 'beauty salon web design', 'appointment booking', 'salon marketing'];
  return {
    title,
    description,
    keywords,
    alternates: {
      canonical: `${siteUrl}/${locale}/industries/salons`,
      languages: { en: `${siteUrl}/en/industries/salons`, es: `${siteUrl}/es/industries/salons`, "x-default": `${siteUrl}/en/industries/salons` },
    },
    ...ogMeta({ locale, title, description, path: '/industries/salons' }),
  };
}

export default async function SalonIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'SalonPage' });
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href={`${siteConfig.whatsapp.link}?text=${encodeURIComponent('Hola, me interesa una página web para mi salón de belleza')}`}
            target="_blank"
      ...
      <RelatedIndustries locale={locale} current="salons" />
      <InternalNav locale={locale} exclude={['/industries']} />
      <Footer />
      <FloatingSocialButtons />
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/salons' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbSalon'), url: `${siteUrl}/${locale}/industries/salons` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
