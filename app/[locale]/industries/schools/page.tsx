...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const keywords = locale === 'es'
    ? ['web para escuelas', 'diseño web educación', 'inscripciones en línea', 'web academia']
    : ['school websites', 'education web design', 'online enrollment', 'academy website'];
  return {
    title,
    description,
    keywords,
    ...
    ...ogMeta({ locale, title, description, path: '/industries/schools' }),
  };
}

export default async function SchoolIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'SchoolPage' });
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href={`${siteConfig.whatsapp.link}?text=${encodeURIComponent('Hola, me interesa una página web para mi escuela')}`}
            target="_blank"
      ...
      <RelatedIndustries locale={locale} current="schools" />
      <InternalNav locale={locale} exclude={['/industries']} />
      <Footer />
      <FloatingSocialButtons />
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/schools' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbSchool'), url: `${siteUrl}/${locale}/industries/schools` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
