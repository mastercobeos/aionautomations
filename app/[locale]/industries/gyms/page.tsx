...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'GymPage' });
  const title = t('metaTitle');
  const description = t('metaDesc');
  const keywords = locale === 'es'
    ? ['web para gimnasios', 'diseño web fitness', 'registro membresías', 'agenda de clases']
    : ['gym websites', 'fitness web design', 'membership registration', 'class scheduling'];
  return {
    title,
    description,
    keywords,
    alternates: {
      canonical: `${siteUrl}/${locale}/industries/gyms`,
      languages: { en: `${siteUrl}/en/industries/gyms`, es: `${siteUrl}/es/industries/gyms`, "x-default": `${siteUrl}/en/industries/gyms` },
    },
    ...ogMeta({ locale, title, description, path: '/industries/gyms' }),
  };
}

export default async function GymIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'GymPage' });
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href={`${siteConfig.whatsapp.link}?text=${encodeURIComponent('Hola, me interesa una página web para mi gimnasio')}`}
            target="_blank"
      ...
      <RelatedIndustries locale={locale} current="gyms" />
      <InternalNav locale={locale} exclude={['/industries']} />
      <Footer />
      <FloatingSocialButtons />
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/gyms' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbGym'), url: `${siteUrl}/${locale}/industries/gyms` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
