...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: { en: `${siteUrl}/en/industries/real-estate`, es: `${siteUrl}/es/industries/real-estate` },
    },
  };
}

export default async function RealEstateIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'RealEstatePage' });
    ...
    <main className="relative min-h-screen bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="mx-auto max-w-2xl px-6 text-center lg:px-8">
          ...
          <a
            href="https://wa.me/13028188842?text=Hola%2C%20me%20interesa%20una%20p%C3%A1gina%20web%20para%20mi%20inmobiliaria"
            target="_blank"
      ...
      <PageSchema schemas={[
        webPageSchema({ locale, title: t('metaTitle'), description: t('metaDesc'), path: '/industries/real-estate' }),
        breadcrumbSchema([
          { name: t('breadcrumbHome'), url: `${siteUrl}/${locale}` },
          { name: t('breadcrumbIndustries'), url: `${siteUrl}/${locale}/industries` },
          { name: t('breadcrumbRealEstate'), url: `${siteUrl}/${locale}/industries/real-estate` },
        ]),
        faqSchema(faqs),
      ]} />
      </div>
